...
def create_device(step, dev_name, entity_name, entity_type, endpoint, protocol):
    ...
    prot = ProtocolTypes.get(protocol)
    req=user_steps.create_device(world.service_name, dev_name, world.service_path, endpoint, {}, entity_name, entity_type, {}, {}, prot, True)
...
def create_device_with_attrs_cmds(step, dev_name, protocol, typ1, typ2, name1, name2, type1, type2, value1, value2):
    ...
    req=user_steps.create_device(world.service_name, dev_name, world.service_path, {}, commands, {}, {}, attributes, st_attributes, prot, True)
    assert req.status_code == 201, 'ERROR: ' + req.text + "El device {} no se ha creado correctamente".format(dev_name)
#    assert req.headers['Location'] == "/iot/devices/"+str(dev_name), 'ERROR de Cabecera: /iot/devices/' + str(dev_name) + ' esperada ' + str(req.headers['Location']) + ' recibida'
    print 'Se ha creado el device {}'.format(dev_name)
    if world.service_path=='void':
...
def create_device_failed(step, dev_name, protocol, typ, name, type1, value):
    ...
    if typ=='ent_name':
        ...
        world.req=user_steps.create_device(world.service_name, dev_name, world.service_path, {}, commands, name, {}, attributes, st_attributes, prot, True)
    elif typ=='protocol':
        if not name=='null':
            prot = ProtocolTypes.get(name)
            if prot:
                world.req=user_steps.create_device(world.service_name, dev_name, world.service_path, {}, commands, {}, {}, attributes, st_attributes, prot, True)
            else:
                world.req=user_steps.create_device(world.service_name, dev_name, world.service_path, {}, commands, {}, {}, attributes, st_attributes, name, True)
        else:
            world.req=user_steps.create_device(world.service_name, dev_name, world.service_path, {}, commands, {}, {}, attributes, st_attributes, {}, True)
    else:        
        world.req=user_steps.create_device(world.service_name, dev_name, world.service_path, {}, commands, {}, {}, attributes, st_attributes, prot, True)
...
def device_created(step, dev_name):
    ...
    if world.protocol:
        assert response['protocol'] == world.protocol, 'Expected Result: ' + world.protocol + '\nObtained Result: ' + response['protocol']
