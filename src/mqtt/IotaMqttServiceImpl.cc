...
void iota::esp::ngsi::IotaMqttServiceImpl::set_resthandle(iota::RestHandle* service_ptr) {
  resthandle_ptr_ = service_ptr;
}

void iota::esp::ngsi::IotaMqttServiceImpl::set_command_service(iota::esp::ngsi::IotaMqttCommands* command_ptr){
...
void iota::esp::ngsi::IotaMqttServiceImpl::extract_command_id(std::string payload, std::string& out_payload,std::string& out_id){
 ...
 // result|ok#cmdid|2238sdf98234987
  ...
  if (cmd_id_pos == std::string::npos){
    ...
    throw new iota::IotaException("Command response not valid","Missing cmd-id",400);
  }

  std::string cmd_id = payload.substr(cmd_id_pos+6,payload.length() - cmd_id_pos);

  std::size_t end_pos = cmd_id.find("#");

  if (end_pos == std::string::npos){
    //if # not found, it means cmdid it's at the end, so we are done with command id.
...
void iota::esp::ngsi::IotaMqttServiceImpl::processCommandResponse(std::string& apikey,std::string& idDevice, std::string& raw_payload){
...
std::string iota::esp::ngsi::IotaMqttServiceImpl::doPublishCB(std::string& apikey, std::string& idDevice,std::string& json) {
  ...
  try {
   ...
   boost::shared_ptr<iota::Device> dev = resthandle_ptr_->get_device(idDevice,
                                  serviceMQTT);
    ...
    cb_response.assign(publisher_ptr->publishContextBroker(ngsi_context_element,
                       vJsonTT, pt_cb, timeInstant));
