...
std::string iota::esp::tt::SearchResponse::doSearchTTResponse(
  ...
  if (cResponses.get_context_responses().size() > 0) {
    ...
    if (cResp.get_code()=="200") {
      ...
      //OK, some context_responses are available, let's match them with our CC_AttributesType
      ...
      for (int i=0; i < attributes.size(); i++) {
        ...
        if (itModule != attributes[i].end() && itBus != attributes[i].end()) {
          ...
          //at this point, I have no idea about how to deal with this module, it could be GC, GM, P1, K, or B
          //each of them may have different expectations about retrieved Attributes from ContextBroker, so I let
...
void iota::esp::tt::TTResponse::setBusId(std::string bus) {
  busid.assign(bus);
}
...
void iota::esp::tt::TTResponse::processContextElement(::iota::ContextElement&
    cElement,CC_AttributesType& attributes) {
...
void iota::esp::tt::TTResponse::populateInternalFields(CC_AttributesType& attribute) {
...
void iota::esp::tt::TTCloseResponse::populateInternalFields(CC_AttributesType&
    attribute) {
...
iota::esp::tt::TTCloseResponse::TTCloseResponse(std::string m) : TTResponse(m) {
}
...
void iota::esp::tt::TTResponse::parseResponse(::iota::Attribute& foundAttribute) {
...
std::string iota::esp::tt::TTResponse::parseMetadatas(::iota::Attribute& attrib) {
...
void iota::esp::tt::GCResponse::populateInternalFields(CC_AttributesType&
    attributes) {
...
void iota::esp::tt::GMResponse::populateInternalFields(CC_AttributesType&
    attributes) {
...
void iota::esp::tt::TTCloseResponse::processContextElement(::iota::ContextElement&
    context,CC_AttributesType& attributes) {
...
iota::esp::tt::BResponse::BResponse(std::string module) : TTCloseResponse(module) {
}
...
std::string iota::esp::tt::BResponse::parseCompoundSimpleValue(
  ...
  if (attribute.get_value_compound().size() > 0) {
    ...
    for (int i=0; i<attribute.get_value_compound().size(); i++) {
      ...
      if (attribute.get_value_compound()[i].get_name()== TT_B_DISCONN_STACK_TIME) {
...
void iota::esp::tt::BResponse::populateInternalFields(CC_AttributesType& attributes) {

  TTResponse::populateInternalFields(attributes);
  nameTTAttribute.assign(module);

  //Now we retrieve the values for the fields that must be included in the response
...
void iota::esp::tt::BResponse::processContextElement(::iota::ContextElement& context,
...
iota::esp::tt::GPSResponse::GPSResponse(std::string module): TTResponse(module) {

}


void iota::esp::tt::GPSResponse::populateInternalFields(CC_AttributesType&
    attribute) {
  nameTTAttribute.assign("position"); //specific for this module, and needed for matching.
