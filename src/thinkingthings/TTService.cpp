...
void iota::esp::TTService::set_option(const std::string& name,
  ...
  if (name.compare("ConfigFile") == 0) {
    ...
    try {
      //TODO read_xml(value, _service_configuration,    boost::property_tree::xml_parser::no_comments);
...
void iota::esp::TTService::start() {
  ...
  //enable_ngsi_service(filters, REST_HANDLE(&iota::esp::TTService::service), this);
}


void iota::esp::TTService::service(pion::http::request_ptr& http_request_ptr,
  ...
  //2. Query the CB to get what I need to response to the device (HTTP Response)
  ...
  try {
    ...
    response.assign(strBuffer);//Just to returns something in case of failure.
    ...
    ESP_Result mainResult = getESPLib()->executeRunnerFromBuffer(idSensor, "main",
                            params, (char*)strBuffer.c_str(), strBuffer.length());

    //get device-id.

    std::string device_id = mainResult.findInputAttributeAsString("request",
    ...
    if (device_id != "") {
      ...
      get_service_by_name(pt_cb,service_tt,
      ...
      std::string temp;
      std::vector<std::string> vJsonTT_Plain; //This would be published to CB if no previous entries are found.
      std::vector<std::string> vJsonTT_Processed; //This is the "normal" processed (with some prefixes) JSON of TT Attributes.
      ...
      if (response == "") {
        ...
        ::iota::ContextElement cElemNew(device_id, "",
        ...
        response.assign(seeker.searchTTResponse(attributes_result,
                                                cElemNew.get_id(), cElemNew.get_type(), queryC));
      ...
      //In anycase, as I have some message from the TT Device, I have to populate the ContextElement and send it to CB.
...
void iota::esp::TTService::add_info(boost::shared_ptr<Service>& pt,
  ...
  try {
    get_service_by_apiKey(pt,
                          apiKey); //TODO: this has to change, as it's not going to work anymore.
...
extern "C" PION_PLUGIN void pion_destroy_TTService(iota::esp::TTService*
    service_ptr) {
  delete service_ptr;
}
