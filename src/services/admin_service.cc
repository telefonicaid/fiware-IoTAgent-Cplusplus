...
bool iota::AdminService::check_device_commands_duplicate(
  ...
  mongo::BSONArrayBuilder arrayCommands;
  ...
  if (objCommands.isValid()) {
    ...
    while (array_eltos.more()) {
      ...
      if (elto.isABSONObj()) {
        ...
        name = eltoBSON.getStringField(NAME);
        if (!name.empty()) {
          arrayCommands.append(name);
        }
      }
    }  // while
  }
