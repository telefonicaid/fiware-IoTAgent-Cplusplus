...
void iota::AdminManagerService::resolve_endpoints(std::vector<DeviceToBeAdded>&
  ...
  */
  ...
  if (document.HasMember("devices")) {
    IOTA_LOG_DEBUG(m_log, "resolve_endpoints: devices found, parsing individually");
    ...
    IOTA_LOG_DEBUG(m_log, "resolve_endpoints: size of elements [" << devices.Size()
                   <<
                   "]");
    try {
      for (rapidjson::SizeType i = 0; i < devices.Size(); i++) {
        ...
        if (devices[i].HasMember("protocol")) {
          ...
          IOTA_LOG_DEBUG(m_log, "resolve_endpoints: Processing first Device: protocol ["
                         <<
                         protocol << "]");
          std::vector <IotagentType> v_endpoint = map_endpoints[protocol];

          if (v_endpoint.size() == 0) {
            IOTA_LOG_DEBUG(m_log, "resolve_endpoints: getting endpoints for protocol [" <<
                           protocol << "]");
            v_endpoint = _service_mgmt.get_iotagents_by_service(service, sub_service,
                         protocol);
            map_endpoints[protocol] =  v_endpoint;
            IOTA_LOG_DEBUG(m_log, "resolve_endpoints: endpoints [" << v_endpoint.size()
                           << "] found for device");
          }
          ...
          for (int j = 0; j < v_endpoint.size(); j++) {
            ...
            IOTA_LOG_DEBUG(m_log, "resolve_endpoints: adding endpoint [" << v_endpoint[j] <<
  ...
  for (int i = 0; i < all_dest.size(); i++) {
    try {
      ...
      if (code_i == pion::http::types::RESPONSE_CODE_OK) {
        ...
        IOTA_LOG_DEBUG(m_log, "get device new limitI " << limitI  << " new offset " <<
                       offsetI);
        ...
        response_from_iotagent_nok.insert(std::pair<std::string, std::string>
                                          (all_dest.at(
                                             i), error_manager(code_i, message)));
      }
    }
    catch (std::exception& e) {
      code_i = -1;
  ...
  while (it_resp != response_from_iotagent.end()) {
    try {
      ...
      if (!it_resp->second.empty()) {
        IOTA_LOG_DEBUG(m_log, "Response " << it_resp->first << " " << it_resp->second);
...
int iota::AdminManagerService::post_multiple_devices(
  ...
  for (int i = 0; i < v_devices_endpoint_in.size(); i++) {
    ...
    int code_i = operation_device_iotagent(url_endpoint,content,service,
                                           sub_service,x_auth_token,pion::http::types::REQUEST_METHOD_POST,temp_res);
    // If no successful response, nothing
    if (code_i == pion::http::types::RESPONSE_CODE_CREATED) {
      response_from_iotagent.insert(std::pair<std::string, std::string>(url_endpoint,
                                    temp_res));
    }
    else {
    ...
    IOTA_LOG_DEBUG(m_log,"Endpoint: ["<< url_endpoint<< "] Result: "
...
int iota::AdminManagerService::delete_device_json(
  ...
  IOTA_LOG_DEBUG(m_log,
                 "delete_device_json: endpoints found [" << v_endpoints.size() << "]");
  int code = delete_multiple_devices(v_endpoints, id_device,
...
int iota::AdminManagerService::put_device_json(
  ...
  if (validate_json_schema(body, devTable,
    ...
    IOTA_LOG_DEBUG(m_log, "put_device_json: endpoints [" << v_endpoint.size()
                   << "] found for device");
...
int iota::AdminManagerService::put_multiple_devices(std::vector<DeviceToBeAdded>&
    v_devices_endpoint_in, const std::string& device_id, std::string service,
  ...
  for (int i = 0; i < v_devices_endpoint_in.size(); i++) {
    ...
    int code_i = operation_device_iotagent(url_endpoint, dev.get_device_json(),
                                           service, sub_service, x_auth_token, pion::http::types::REQUEST_METHOD_PUT,
                                           temp_res);
    // If no successful response, nothing
    if (code_i == pion::http::types::RESPONSE_CODE_NO_CONTENT) {
      response_from_iotagent.insert(std::pair<std::string, std::string>(url_endpoint,
...
int iota::AdminManagerService::post_protocol_json(
  ...
  if (body.empty()) {
    ...
    if (element.eoo()) {
      IOTA_LOG_DEBUG(m_log, "Protocol: " + protocol_name + " has not got services");
    }
    else {
      ...
      for (unsigned int i = 0; i < be.size(); i++) {
        ...
        if (it == services_in_mongo.end() ||
          ...
          IOTA_LOG_DEBUG(m_log, "services changed, update:"+ srv + " " + srv_path);
...
int iota::AdminManagerService::post_service_json(
    ...
    for (int j = 0; j < obj_protocols.size(); j++) {
      ...
      for (int i = 0; i < all_dest.size(); i++) {
        try {
          ...
          if (code_i == pion::http::types::RESPONSE_CODE_CREATED) {
            response_from_iotagent.insert(std::pair<std::string, std::string>(all_dest.at(
                                            i).endpoint, resp_http->get_content()));
          }
          else {
            std::string message;
            if (code_i != -1) {
            ...
            response_from_iotagent_nok.insert(std::pair<std::string, std::string>
                                              (all_dest.at(
                                                 i).endpoint, error_manager(code_i, message)));
          }
        }
        catch (std::exception& e) {
  ...
  IOTA_LOG_DEBUG(m_log, param_request << " status=" << code << " reason=" <<
                 response);
  http_response.set_status_code(code);
  http_response.set_status_message(iota::Configurator::instance()->getHttpMessage(
                                     code));
...
void iota::AdminManagerService::add_errors_to_response(std::string& response,
