...
std::string iota::tt::TTCBPublisher::buildcbURL(
  ...
  if (!cb_url.empty()) {
    ...
    if (!updateContex.empty()) {
      ...
      //Not sure if this will work the way I thought, like getting updateContext from the service in first place.
      const JsonValue& ngsi_urls = iota::Configurator::instance()->get("ngsi_url");
