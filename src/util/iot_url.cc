...
iota::IoTUrl::IoTUrl(std::string url):
  _port(-1) {

   if (url.empty()){
      throw iota::IotaException(iota::types::RESPONSE_MESSAGE_INVALID_PARAMETER + "[ empty url ]",
                              "empty url",
...
std::string iota::IoTUrl::getProtocol(void) {
  return (_protocol);
}

std::string iota::IoTUrl::getHost(void) {
  return (_host);
}

int iota::IoTUrl::getPort(void) {
  return (_port);
}

std::string iota::IoTUrl::getPath(void) {
  return (_path);
}

std::string iota::IoTUrl::getQuery(void) {
  return (_query);
}
