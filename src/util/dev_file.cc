...
void iota::DevicesFile::parse_to_cache(Cache* pcache) {
  ...
  try {
    if (_document.IsObject() && _document.HasMember(iota::store::types::DEVICES.c_str())
        && _document[iota::store::types::DEVICES.c_str()].IsArray()) {
      const iota::JsonValue& devices = _document[iota::store::types::DEVICES.c_str()];
      for (rapidjson::SizeType i = 0; i < devices.Size(); i++) {
        ...
        if (!status.empty() && (status.compare(iota::store::types::INACTIVE.c_str()) ==0)) {
          item->_active = false;
        }
        else {
          item->_active = true;
        ...
        if (devices[i].HasMember(iota::store::types::COMMANDS.c_str())) {
          const iota::JsonValue& commands = devices[i][iota::store::types::COMMANDS.c_str()];
          for (rapidjson::SizeType j = 0; j < commands.Size(); j++) {
            ...
            item->_commands.insert(std::pair<std::string, std::string>(attr_name,
                                   attr_cmd));
        ...
        if (devices[i].HasMember(iota::store::types::ATTRIBUTES.c_str())) {
          const iota::JsonValue& attributes = devices[i][iota::store::types::ATTRIBUTES.c_str()];
          for (rapidjson::SizeType j = 0; j < attributes.Size(); j++) {

            std::string attr_name, attr_attr, attr_type;
            if (attributes[j].HasMember(iota::store::types::ATTRIBUTE_ID.c_str())) {
            ...
            item->_attributes.insert(std::pair<std::string, std::string>(attr_name,
                                     json));
          }
        }

        // Static attributes
        if (devices[i].HasMember(iota::store::types::STATIC_ATTRIBUTES.c_str())) {
          ...
          for (rapidjson::SizeType j = 0; j < static_attributes.Size(); j++) {
            ...
            item->_static_attributes.insert(std::pair<std::string, std::string>(attr_name,
                                            buffer.GetString()));
          }
        }
