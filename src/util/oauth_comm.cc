...
void iota::OAuth::set_timeout(int timeout) {
  _timeout = timeout;
}

int iota::OAuth::get_timeout() {
  return _timeout;
}

std::string iota::OAuth::get_oauth_validate() {
  return _oauth_validate;
}

std::string iota::OAuth::get_oauth_roles() {
  return _oauth_roles;
}

std::string iota::OAuth::get_oauth_projects() {
  return _oauth_projects;
}

std::string iota::OAuth::get_oauth_trust() {
  return _oauth_trust;
}
...
boost::property_tree::ptree iota::OAuth::get_auth_data() {
  return _auth;
}
...
std::string iota::OAuth::get_trust_token() {
  return _trust_token;
}
...
void iota::OAuth::receive_event_renew_token(
  ...
  if (response.get() != NULL &&
      (response->get_status_code() == pion::http::types::RESPONSE_CODE_OK ||
       ...
       response->get_status_code() == pion::http::types::RESPONSE_CODE_CREATED) &&
...
std::string iota::OAuth::receive_event_get_user(
  ...
  if (!ptree_user.empty()) {
    ...
    std::string domain = ptree_user.get<std::string>("token.user.domain.name", "");
    if (response->get_status_code() == pion::http::types::RESPONSE_CODE_OK
        && !user_id.empty() && !domain.empty()
        && domain.compare(_domain) == 0) {
      _user_id = user_id;
...
void iota::OAuth::renew_token(std::string scope) {
  ...
  pion::http::request_ptr request =
    create_request(compound_server, resource, c.str(), query,
                   additional_info);
...
std::string  iota::OAuth::get_user(std::string token, std::string token_pep) {
  ...
  str_response = send_request(_oauth_validate, "GET", "", "", headers, "get_user",
                              boost::bind(&iota::OAuth::receive_event_get_user, shared_from_this(), _1, _2,
                                          _3));
...
std::string iota::OAuth::get_subservice(std::string domain,
  ...
  headers.put(HTTP_HEADER_AUTH, token_pep);
  str_response = send_request(_oauth_projects,
                              pion::http::types::REQUEST_METHOD_GET, "", query, headers,
                              "get_subservice",
                              boost::bind(&iota::OAuth::receive_event_get_subservice, shared_from_this(), _1,
                                          _2, _3));
  return str_response;
}

boost::property_tree::ptree iota::OAuth::validate_user_token(std::string token,
    iota::OAuth::app_callback_t callback) {
...
boost::property_tree::ptree iota::OAuth::get_user_roles(std::string user_id) {
  ...
  str_response = send_request(_oauth_roles, pion::http::types::REQUEST_METHOD_GET,
...
pion::http::request_ptr iota::OAuth::create_request(std::string server,
    std::string resource,
...
std::string iota::OAuth::get_identifier() {
  return _id;
}
