...
class Collection {
  public:
    ...
    struct Options {
      ...
      enum _v {
        ...
        Secondary = mongo::QueryOption_NoCursorTimeout | mongo::QueryOption_SlaveOk
      };
    };
    ...
    int count(const mongo::BSONObj& query,
              int retry = 0) ;

    int remove(const mongo::BSONObj& query,
               int retry = 0);
    ...
    const std::string& getBBDD() const throw () {
      return a_bbdd;
    }

    void setBBDD(const std::string& value) throw () {
      a_bbdd = value;
    }
  ...
  protected:
    ...
    int getLastError(
      const std::string& bbdd, const mongo::BSONObj &obj);
