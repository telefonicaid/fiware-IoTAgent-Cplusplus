...
void iota::ULInsertObservation::translate(std::string str_io,
  ...
  // a nivel de campo dentro de una medida, para que sea posible recibir en un mensaje
  ...
  for (i = 0; i < tokens_msgs.size(); i++) {
    ...
    if (protocol == 0) {    // SENSOR_PROTOCOL_SML = 0
      tokens_msgs[i] = contentForSBCProtocol(tokens_msgs[i],
                                             samplingTime, device_name);
    }
    ...
    std::string sep(".");
    std::vector<std::string> v_tokens = riot_tokenizer(tokens_io[UL_ID_RESOURCE],
                                        sep);
    ...
    try {
      ...
      if (n_dates == 0) {
        ...
        std::string s_time_e;
        try {
          long s_time_number = boost::numeric_cast<long>(boost::lexical_cast<double>
                               (stime_tokens[0]));
          iota::RiotISO8601 iso_b(s_time_number);
          s_time_b = iso_b.toString();
          s_time_number += boost::numeric_cast<long>(boost::lexical_cast<double>
                           (stime_tokens[1]));
...
std::string iota::ULInsertObservation::contentForSBCProtocol(std::string io,
  ...
  */
  ...
  if ((p_pipe != std::string::npos) &&
      (p_pipe_f != std::string::npos)) {
    if (p_pipe == p_pipe_f) {
