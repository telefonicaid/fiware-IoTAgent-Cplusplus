...
void iota::UL20Service::service(pion::http::request_ptr& http_request_ptr,
                                std::map<std::string, std::string>& url_args,
                                std::multimap<std::string, std::string>& query_parameters,
                                pion::http::response& http_response, std::string& response) {
...
int iota::UL20Service::sendHTTP(const std::string& endpoint, int port,
  ...
  IOTA_LOG_DEBUG(m_logger,"UL20Plugin sendHTTP(): " <<
                 ...
                 " body:"<<  message << " outgoing_route:" << outgoing_route);
  ...
  try {
    ...
    if (callback) {
      ...
      http_client->async_send(request, timeout,
                              proxy_server, callback);
    }
    else {
      http_client.reset(new iota::HttpClient(endpoint, port));
      pion::http::response_ptr response_ptr = http_client->send(request, timeout,
                                              proxy_server);
      if (response_ptr.get() != NULL) {
        ...
        if ((response_ptr->get_content_length() != 0)
          ...
          response.assign(pion::algorithm::url_decode(response_ptr->get_content()));
          boost::trim(response);
...
int iota::UL20Service::execute_command(const std::string& destino,
  ...
  if (!callback) {
    std::string command_response;
    std::string id_command;
    int code = isCommandResp(content_resp, code_resp, command_response, id_command);
    if (code < 0) {
  ...
  try {
    ...
    if (item_dev != NULL) {
      ...
      if (((item_dev->_registration_id).empty())
        ...
        for (p = item_dev->_commands.begin(); p!=item_dev->_commands.end(); ++p) {
...
std::string iota::UL20Service::get_ngsi_operation(const std::string&
    operation) {
...
extern "C" PION_PLUGIN void pion_destroy_UL20Service(iota::UL20Service*
    service_ptr) {
  delete service_ptr;
}
