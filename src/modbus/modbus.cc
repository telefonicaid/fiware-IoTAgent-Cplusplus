...
bool iota::Modbus::receive_modbus_frame(const std::vector<unsigned char>&
  ...
  if ((_modbus_frame.size() == 0) || !check_crc(mb_msg)) {
    // No request or bad CRC
    frame_ok = false;
    error_frame = "Bad modbus crc or not associated request";
  }
  else {
    try {
      ...
      if (mb_msg.at(0) != _slave_address ||
        ...
        frame_ok = false;
        error_frame = "Bad modbus slave address or function code are not requested";
      }
      else {
        if (_function_code == iota::Modbus::READ_HOLDING_REGISTERS) {
          unsigned short bytes = mb_msg.at(2);
          unsigned short num_reg = 0;
          int i = 3;
          IOTA_LOG_DEBUG(m_logger,
                         "Reading " + boost::lexical_cast<std::string>(bytes) + " bytes from position " +
          ...
          IOTA_LOG_DEBUG(m_logger,
                         "Read " + boost::lexical_cast<std::string>(num_reg) + " registers");
        }
        else if (_function_code == iota::Modbus::PRESET_SINGLE_REGISTER) {
          unsigned short addr_response = (mb_msg.at(2) << 8) | mb_msg.at(3);
          unsigned short value = (mb_msg.at(4) << 8) | mb_msg.at(5);
          if (addr_response != _address_data || value != _number_of_or_value) {
            frame_ok = false;
            error_frame = "Modbus preset single register response is not request echo";
          }
        }
      }
    }
...
bool iota::Modbus::check_crc(const std::vector<unsigned char>& frame) {
  ...
  unsigned short received_crc = (frame.at(frame.size() - 2) << 8) | frame.at(
                                  frame.size() -1);
