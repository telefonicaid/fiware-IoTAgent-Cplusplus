...
std::string& iota::Process::get_url_base() {
  return _url_base;
}
...
pion::http::plugin_server_ptr iota::Process::add_http_server(
  ...
  if (_http_servers.size() == 0) {
    if (endpoint.empty()) {
      ...
      endpoint.append(":");
      endpoint.append(boost::lexical_cast<std::string>(http_server->get_port()));
    }
    else {
      boost::asio::ip::tcp::endpoint e = get_endpoint(endpoint);
      http_server.reset(new pion::http::plugin_server(
...
pion::http::plugin_service* iota::Process::get_service(std::string
    http_resource) {
