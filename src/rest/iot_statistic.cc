...
iota::IoTStatistic::IoTStatistic(const std::string& name): _name(name),
  _enabled(true) {
  //ctor
}
...
const std::string& iota::IoTStatistic::get_name() const {
  return _name;
}

void iota::IoTStatistic::set_enable(bool enabled) {
  _enabled = enabled;
}

const bool iota::IoTStatistic::get_enable() const {
  return _enabled;
}

//iota::IoTStatistic::iota::IoTStatistic& iota::IoTStatistic::operator=(iota::IoTStatistic::iota::IoTStatistic& stat) {
...
iota::IoTStatistic::iot_accumulator_ptr iota::IoTStatistic::operator[](
  ...
  if (it != _accumulators_.end()) {
    ...
    std::map<std::string, iota::IoTStatistic::iot_accumulator_ptr>::iterator it_acc =
      it->second.begin();
    ...
    if (it_acc == it->second.end()) {
      ...
      return (it->second.insert(
                std::make_pair<std::string, iota::IoTStatistic::iot_accumulator_ptr>(acc_name,
                    acc))).first->second;
    }
    else {
    ...
    accs.insert(std::make_pair<std::string, iota::IoTStatistic::iot_accumulator_ptr>
                (acc_name, acc));
    return (_accumulators_.insert(
              std::make_pair<long, std::map<std::string, iota::IoTStatistic::iot_accumulator_ptr> >
              (acc_time, accs))).first->second[acc_name];
...
void iota::IoTStatistic::add(const std::string& acc_name, double value) {
  ...
  if (it != _accumulators_.end()) {
    ...
    if (it_acc != it->second.end()) {
      ...
      (*acc)(value);
      it->second.insert(
        std::make_pair<std::string, iota::IoTStatistic::iot_accumulator_ptr>(acc_name, acc));
    }
  }
  else {
    reset(acc_time);
...
const std::map<long, std::map<std::string, iota::IoTStatistic::iot_accumulator_ptr> >&
...
/**
 * remove last 24 hours accumulate data
 **/
