...
class Configurator {
  public:
    ...
    /**
      * Transform a Json object in first-level configuration into a std::string map
      * @param Field in first level. This field only can contain simple fields (string, boolean, number).
      * @param map<std::string, std::string> where transformation is stored. You can get a value by to_map[<field>].
      */
    void get(const std::string& field, std::map<std::string, std::string>& to_map);
  ...
  private:
    ...
    boost::shared_ptr<Resource> check_resource(boost::shared_ptr<JsonDocument>
        resource_obj, std::string& error);
