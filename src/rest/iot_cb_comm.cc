...
const std::string iota::ContextBrokerCommunicator::NUMBER_OF_TRIES = "number_of_tries";
...
void iota::ContextBrokerCommunicator::receive_event(
  ...
  int number_of_tries = additional_info.get<int>(iota::ContextBrokerCommunicator::NUMBER_OF_TRIES, 0);
  IOTA_LOG_DEBUG(m_logger, "url=" << url << " error=" << error << " tries=" << number_of_tries);
  ...
  if ((!error) && (response.get() != NULL)) {
    ...
    if (response->get_status_code() ==
        ...
        number_of_tries > 0) {
      additional_info.put(iota::ContextBrokerCommunicator::NUMBER_OF_TRIES, ++number_of_tries);
...
std::string iota::ContextBrokerCommunicator::send(std::string url,
  ...
  try {
    ...
    IOTA_LOG_DEBUG(m_logger, "send Server:" << server <<
                   "service:" << service << " service_path:" << service_path << " tries=" << number_of_tries);
