...
class RestHandle: public pion::http::plugin_service,
  ...
  public:
    ...
    boost::shared_ptr<IoTStatistic> add_statistic_counter(const std::string&
        name_stat, bool enable);
    boost::shared_ptr<IoTStatistic> get_statistic_counter(const std::string&
        name_stat);
    void reset_counters();
    ...
    /**
       * @name    getDevice
       * @brief   get Device from cache, check active  and autocomplete, first look for entity
       ...
       * @endcode
       */
    const boost::shared_ptr<Device> get_device(const std::string& name,
        const std::string& service = std::string() ,
        const std::string& service_path = std::string());

    const boost::shared_ptr<Device> get_device_by_entity(const std::string& name,
  ...
  private:
    ...
    /**
       ...
       * if there is more than one throw excepciton
       **/
    const JsonValue& get_service_by_apiKey_file(boost::property_tree::ptree& pt_cb,
        const std::string& apiKey);
    ...
    /**
       ...
       * if there is more than one throw excepciton
       **/
    const JsonValue& get_service_by_name_file(boost::property_tree::ptree& pt_cb,
        const std::string& name,
        const std::string& service_path = std::string());
