...
std::string
ESP_StringUtils::AddEndingString(
...
std::string ESP_StringUtils::strToLower(std::string str) {
  return str;
}

std::string ESP_StringUtils::strToUpper(std::string str) {
  return str;
}

std::string ESP_StringUtils::strToFirstCapital(std::string str) {
  return str;
}
...
bool ESP_StringUtils::isValidLogin(std::string str) {
  for (unsigned int i=0; i < str.length(); i++) {
    if (!((str.at(i) >= 'A' && str.at(i) <= 'Z') || (str.at(i) >= 'a' &&
          str.at(i) <= 'z') || (str.at(i) >= '0' && str.at(i) <= '9'))) {
      return false;
    }
  }
  return true;
...
std::string ESP_StringUtils::condense(std::string str) {  unsigned int iniSpaceString = std::string::npos;
...
CC_ParamsType ESP_StringUtils::parseParamsString(std::string str,
  ...
  for (int i=0; i<ST.countElements(); i++) {
    ...
    if (ST2.countElements() == 2) {
      pt.insert(CC_ParamsPair(ESP_StringUtils::strToLower(ST2.elementAt(0)),
                              valueToLower ? ESP_StringUtils::strToLower(ST2.elementAt(1)) : ST2.elementAt(
                                1)));
    }
  }
...
int CC_StringTokenizer::countElements() {
  return count;
}
