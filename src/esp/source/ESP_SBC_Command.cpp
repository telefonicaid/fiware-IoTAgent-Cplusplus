...
std::string ESP_SBC_Command::getCommandName() {
  return name;
}

std::string ESP_SBC_Command::getCommandID() {
  return cmd_id;
}

std::string ESP_SBC_Command::getDestination() {
  return dest;
}
  ...
  for (; pElem; pElem=pElem->NextSiblingElement()) {
    ...
    if (pChild) {
      ...
      if (pGrandson && pGrandson->GetText()>0) {
        std::string value = std::string(pGrandson->GetText(),
                                        strlen(pGrandson->GetText()));
...
std::string ESP_SBC_Command::getUL20output() {
  return outputUL20;
}

bool ESP_SBC_Command::isParseCorrect() {
  return parseCorrect;
}
