...
void ESP_Attribute::setValue(std::string name, const char* value, int datasize,
  ...
  switch (datatype) {
    ...
    case ESP_DataType::ESP_DataType_STRING: {
        ...
        //this->_datasize++; // the \0 is there, but do not take it into account for the size
        break;
      }
    default:
      break;
...
/*
void ESP_Attribute::setValue(std::string name, void *value, int datasize, int datatype, int source, int dest)
{
    ...
    // String Type
    else if (datatype == ESP_DataType::ESP_DataType_STRING || datatype/10 == ESP_DataType::ESP_DataType_STRING)
    {
    ...
    if (this->datatype > 10) this->datatype = this->datatype / 10; // Get Basic Type
...
int ESP_Attribute::compareValue(ESP_Attribute* pa1, ESP_Attribute* pa2) {
  if (pa1 != NULL && pa2 != NULL && pa1->_datatype == pa2->_datatype) {
    if (pa1->_datatype == ESP_DataType::ESP_DataType_INT) {
      return pa1->getIntValue() < pa2->getIntValue() ? -1 : pa1->getIntValue() >
             pa2->getIntValue() ? 1 : 0;
    }
    else if (pa1->_datatype == ESP_DataType::ESP_DataType_FLOAT) {
      return pa1->getFloatValue() < pa2->getFloatValue() ? -1 : pa1->getFloatValue() >
             pa2->getFloatValue() ? 1 : 0;
    }
    else if (pa1->_datatype == ESP_DataType::ESP_DataType_LONG) {
      return pa1->getLongValue() < pa2->getLongValue() ? -1 : pa1->getLongValue() >
             pa2->getLongValue() ? 1 : 0;
    }
    else if (pa1->_datatype == ESP_DataType::ESP_DataType_DOUBLE) {
      return pa1->getDoubleValue() < pa2->getDoubleValue() ? -1 :
...
int ESP_Attribute::getIntValue() {
  return (_value)?*(int*)_value:0;
}
...
std::string ESP_Attribute::getName() {
  return _name;
}
...
ESP_Attribute* ESP_Attribute::searchAttributeRefByName(CC_AttributesType*
