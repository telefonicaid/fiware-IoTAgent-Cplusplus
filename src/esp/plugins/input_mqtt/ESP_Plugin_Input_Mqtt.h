...
class ESP_Plugin_Input_Mqtt : public ESP_Plugin_Input_Base {
  ...
  public:
    ...
    int initMQTT(int port, std::string host,std::string topicPub,
                 std::string topicSub,std::string user, std::string passwd,bool cleanSession,
...
class ESP_Input_Mqtt : public ESP_Input_Base {
  public:
    ...
    int type;                       //Not sure if this makes sense here, as there will be Publishers/Subscribers classes?

    bool cleanSession;              //false to tell broker we want anything that is available for us.
