...
ESP_MqttWrapper::ESP_MqttWrapper(IMosquitto* mqtt,int port, std::string host,
                                 std::string topicPub,std::string topicSub,std::string user,std::string passwd) {
  this->port = port;
  this->host = host;
...
void ESP_MqttWrapper::doConnectHost() {
  ...
  if (MOSQ_ERR_SUCCESS == res) {
    CC_Logger::getSingleton()->logDebug("LIB MQTT connecting to host %s:%d.... DONE",
                                        host.c_str(),port);
...
//Do I need to synchronize the read of bytes that happens "asynchronously" and the retrieval of
//the buffer?
...
int ESP_MqttWrapper::publishMaskTopic(const char* buffer, int len) {
  ...
  if (status == StatusType::MQTT_CONNECTED) {
    ...
    CC_Logger::getSingleton()->logDebug("MQTT Publish: %s",std::string(buffer,
                                        len).c_str());
    ...
    if (tpos != std::string::npos) {
      ...
      payload = std::string(buffer+tpos+token.length(),len-tpos-token.length());
    }
...
ESP_MqttWrapper::~ESP_MqttWrapper() {
  ...
  */
  if (NULL != mqttObj) {
    delete mqttObj; //It seems there's no better place to delete IMosquitto object.
    mqttObj = NULL;
  }
}
...
void ESP_MqttWrapper::on_publish(int mid) {
  CC_Logger::getSingleton()->logDebug("Message Successfully Published: %d",mid);
}
...
void ESP_MqttWrapper::on_message(const struct mosquitto_message* message) {
  ...
  if (message != NULL) {
    ...
    CC_Logger::getSingleton()->logError("Message could not be retrieved... ERROR");
  }
}

void ESP_MqttWrapper::on_subscribe(int mid, int qos_count,
                                   const int* granted_qos) {
  CC_Logger::getSingleton()->logDebug("Subscription done: %d granted QOS: %d",mid,
                                      *granted_qos);
}
...
ESP_Input_Base* ESP_Plugin_Input_Mqtt::createInput(TiXmlElement* element) {
  ...
  //CC_Logger::getSingleton()->logError("Type not supported by this plugin: %s",type.c_str());
  return result;
}
...
int ESP_Plugin_Input_Mqtt::initMQTT(int port, std::string host,
  ...
  #else
    return -1; //abort the creation of the service. If MQTT lib is not used, nothing else should be allowed to be done.
  #endif // USE_MQTT
  }
  else {
...
int ESP_Plugin_Input_Mqtt::publish(int idClient, const char* buffer,int len) {
  //CC_Logger::getSingleton()->logDebug("Plugin Input MQTT: just about to publish %d bytes",len);
  if (NULL != buffer) {
    ...
    if (mqttObj) {
      ...
      if (res > 0) {
        CC_Logger::getSingleton()->logDebug("Plugin Input MQTT:  bytes published: %d.... ",
                                            res);
      }
      return res;
    }
    else {
      CC_Logger::getSingleton()->logError("Plugin Input MQTT: no object found to publish");
      return 0;
    }
...
void ESP_MqttWrapper::doSubscribe() {
  ...
  if (MOSQ_ERR_SUCCESS == res) {
    CC_Logger::getSingleton()->logDebug("Lib MQTT: subscribing to topic %s ... DONE ID: %d",
                                        topicSubscribe.c_str(),id);

  }
  else {
    CC_Logger::getSingleton()->logError("Lib MQTT: subscribing to topic %s .... ERROR",
                                        topicSubscribe.c_str());
...
void ESP_MqttWrapper::stopSubscription() {
  ...
  if (canISubscribe()) {
    CC_Logger::getSingleton()->logDebug("Lib MQTT: unsubscribing of topic %s .... ",
                                        topicSubscribe.c_str());
    int id = 0;
    //int res = unsubscribe(&id,topicSubscribe.c_str());
    int res = mqttObj->mqttUnsubscribe(&id,topicSubscribe.c_str());
    if (MOSQ_ERR_SUCCESS == res) {
      CC_Logger::getSingleton()->logDebug("Lib MQTT: unsubscribing of topic %s .... DONE",
                                          topicSubscribe.c_str());
    }
    else {
      CC_Logger::getSingleton()->logError("Lib MQTT: unsubscribing of topic %s .... ERROR (disconnecting anyway)",
                                          topicSubscribe.c_str());
    }
  }
}
...
void ESP_Plugin_Input_Mqtt::stopBroker(int port) {
  CC_Logger::getSingleton()->logDebug("number of running brokers: %d",nBrokers);
  nBrokers--;
  if (nBrokers == 0) {
...
int ESP_Input_Mqtt::openServer() {
  this->_sid = ESP_Plugin_Input_Mqtt::getInstance()->initMQTT(port,host,
               topicPublish,topicSubscribe,user,passwd,cleanSession,_name);
  return this->_sid;
}
...
int ESP_Input_Mqtt::openClient() {
  if (_sid < 0) {
    return ESP_Plugin_Input_Mqtt::getInstance()->initMQTT(port,host,topicPublish,
           topicSubscribe,user,passwd,cleanSession,_name);
  }
  else {
...
bool ESP_Input_Mqtt::stopClient(int id) {
  return true;
}
...
int ESP_Input_Mqtt::readClient(int id, char* buffer, int len) {
  ...
  if (this->_sid < 0) {
    ndata = ESP_Plugin_Input_Mqtt::getInstance()->readfromSubscription(id,buffer,
            len);
  }
  else {
...
void ESP_Input_Mqtt::parseCustomElement(TiXmlElement* element) {
  ...
  std::string cSession = ESP_XmlUtils::queryStringValue(element,"clean-session");
...
int ESP_Input_MqttBroker::openServer() {
  this->_sid = ESP_Plugin_Input_Mqtt::getInstance()->initBroker(port,
               pathToBroker.c_str(),pathToConfig.c_str());
  return this->_sid;
}

int ESP_Input_MqttBroker::acceptServer() {
  ESP_MQTT_Broker* mqttBroker = ESP_Plugin_Input_Mqtt::getInstance()->findBroker(
                                  _sid);
  if (mqttBroker != NULL) {
...
int ESP_Input_MqttBroker::openClient() {
  return -1;
}

bool ESP_Input_MqttBroker::stopClient(int id) {
  return true;
}

bool ESP_Input_MqttBroker::closeClient(int id) {
  return false;
}
