...
int ESP_MqttBuffer::getSerialized(char* buffOutput,int lenRead) {
  ...
  //Copy first message
  //nMsgLen is the actual length of bytes to be copied, same result as calling getRemainingLength()
  //but it might be greater than lenRead, in that case, it means there will be some bytes
...
ESP_MqttMsg::ESP_MqttMsg(const struct mosquitto_message* msg) {
  ...
  length = msg->payloadlen + topicLen + strlen(
             ESP_MQTT_DELIM_END); //extra tail chars at the end of topic
...
ESP_MqttMsg::~ESP_MqttMsg() {
  free(buffSerialized);
}

int ESP_MqttMsg::getTotalLength() {
  return length;
}

int ESP_MqttMsg::getRemainingLength() {
  return length - readOffset;
}
...
void ESP_MqttMsg::moveOffset(int nbytes) {
  readOffset += nbytes;
}
