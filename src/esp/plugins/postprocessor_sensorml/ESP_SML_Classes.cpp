...
ESP_SML_Map::ESP_SML_Map() {
  smlObjectWithTimestamp = NULL;
}

void ESP_SML_Map::transformRefToValue(std::string ref,std::string value) {
  //Working with the multimap

  std::pair<std::multimap<std::string,ESP_SML_Base*>::iterator , std::multimap<std::string,ESP_SML_Base*>::iterator>
  it;
...
void ESP_SML_Map::resetValues() {
  for (std::multimap<std::string, ESP_SML_Base*>::iterator it = mapTrans.begin();
       it != mapTrans.end(); it++) {
    it->second->setValue("");
  }
}
...
void ESP_SML_Map::computeTimestamp() {
  if (smlObjectWithTimestamp != NULL) {
    ...
    std::string strValue = timeStamp.formatUTCTimestamp(year,month,day,h,m,s);
...
/**
* This method is the one which must be changed in order to add more dataTypes to SensorML
output as it will translate XML elements into rigth ESP_SML_Transformer derived classs.
*/
ESP_SML_Base* ESP_SML_Base::translateObject(TiXmlElement* element,
    std::string type,ESP_SML_Map* mapT) {
...
void ESP_SML_Base::setDefinition(TiXmlElement* element) {
  std::string definition = ESP_XmlUtils::queryStringValue(element,
                           SML_XML_DEFINITION);
...
TiXmlElement*  ESP_SML_Base::getTransformedXML() {
  return xmlConverted;
}
...
void ESP_SML_Base::setXlink(TiXmlElement* element) {
  std::string xlkink = ESP_XmlUtils::queryStringValue(element,SML_XML_XLINK_HREF);
  if (xlkink.length()>0) {
...
ESP_SML_List::ESP_SML_List(TiXmlElement* elemWithChildren,
                           ESP_SML_Map* mapT):ESP_SML_Base(elemWithChildren,mapT) {
  nPos=0;
}


ESP_SML_List::ESP_SML_List(TiXmlElement* elemWithChildren,ESP_SML_Map* mapT,
                           const char* xmlType):ESP_SML_Base(elemWithChildren,mapT) {
  nPos=0;
  createXml(xmlType);
...
void ESP_SML_List::parseElement(TiXmlElement* element) {
  ...
  for (TiXmlElement* e = element->FirstChildElement(SML_TRANSFORMER);
       e!= NULL; e = e->NextSiblingElement(SML_TRANSFORMER)) {
...
ESP_SML_Keyword::ESP_SML_Keyword() {
  bFound = false;
}

bool ESP_SML_Keyword::canIPrint() {
  return bFound;
}

void ESP_SML_Keyword::setKeyword(std::string key) {
  keyword = key;
}

std::string ESP_SML_Keyword::getKeyword() {
  return keyword;
}

void ESP_SML_Keyword::setFound(bool found) {
  bFound = found;
}
...
void ESP_SML_Keyword::resetFound() {
  bFound = false;
}


ESP_SML_Unit::ESP_SML_Unit() {
  xmlText = new TiXmlText("");
}
...
TiXmlText* ESP_SML_Unit::getText() {
  return xmlText;
}
...
ESP_SML_DataRecord::ESP_SML_DataRecord(TiXmlElement* inputElement,
                                       ESP_SML_Map* mapT):ESP_SML_List(inputElement,mapT,SML_XML_DATARECORD) {
}

ESP_SML_Field::ESP_SML_Field(TiXmlElement* element,
                             ESP_SML_Map* mapT):ESP_SML_List(element,mapT,SML_XML_FIELD) {
  setName(element);
  setXlink(element);
}
...
ESP_SML_SamplingTime::ESP_SML_SamplingTime(TiXmlElement* inputElem,
    ESP_SML_Map* mapT):ESP_SML_Base(inputElem,mapT) {
...
ESP_SML_Procedure::ESP_SML_Procedure(TiXmlElement* elem,
                                     ESP_SML_Map* mapT):ESP_SML_Base(elem,mapT) {
...
ESP_SML_SensorID::ESP_SML_SensorID(TiXmlElement* elem,
                                   ESP_SML_Map* mapT):ESP_SML_Base(elem,mapT) {
...
ESP_SML_InsertObservation::ESP_SML_InsertObservation(TiXmlElement*
    elemWithChildren,ESP_SML_Map* mapT):ESP_SML_List(elemWithChildren,mapT) {
...
void ESP_SML_InsertObservation::parseElement(TiXmlElement* element) {
  ...
  ESP_SML_SensorID* smlSensor = new ESP_SML_SensorID(element->FirstChildElement(
        SML_SENSORID),mapTrans);
  addSMLElement(smlSensor);

  ESP_SML_Observation* smlObs = new ESP_SML_Observation(element,mapTrans);


  //XML elements needed to create om:Observation element within SensorML are at the
...
ESP_SML_Result::ESP_SML_Result(TiXmlElement* elemWithChildren,
                               ESP_SML_Map* mapT):ESP_SML_List(elemWithChildren,mapT,SML_XML_RESULT) {

}

ESP_SML_Observation::ESP_SML_Observation(TiXmlElement* elem,
    ESP_SML_Map* mapT):ESP_SML_List(elem,mapT) {
...
void ESP_SML_Observation::parseElement(TiXmlElement* element) {
  ...
  */
  ...
  ESP_SML_Result* smlResult = new ESP_SML_Result(element->FirstChildElement(
        SML_RESULT),mapTrans);
...
ESP_SML_Quantity::ESP_SML_Quantity(TiXmlElement* inputElement,
                                   ESP_SML_Map* mapT):ESP_SML_Base(inputElement,mapT) {
  //Example: <transformer type="Quantity" ref="valor" definition="" uom="unit" qrange="0 99999" />
...
ESP_SML_Boolean::ESP_SML_Boolean(TiXmlElement* inputElement,
                                 ESP_SML_Map* mapT):ESP_SML_Base(inputElement,mapT) {
...
ESP_SML_Text::ESP_SML_Text(TiXmlElement* inputElem,
                           ESP_SML_Map* mapT):ESP_SML_Base(inputElem,mapT) {
...
ESP_SML_Vector::ESP_SML_Vector(TiXmlElement* elem,
                               ESP_SML_Map* map):ESP_SML_List(elem,map,SML_XML_VECTOR) {
  setDefinition(elem);
}

ESP_SML_Coordinate::ESP_SML_Coordinate(TiXmlElement* elem,
                                       ESP_SML_Map* map):ESP_SML_Quantity(elem,map) {
...
ESP_SML_Position::ESP_SML_Position(TiXmlElement* inputElem,
                                   ESP_SML_Map* map):ESP_SML_List(inputElem,map) {
  ...
  referenceFrame = ESP_XmlUtils::queryStringValue(inputElem,
                   SML_XML_REFERENCEFRAME);
...
void ESP_SML_Position::parseElement(TiXmlElement* inputElem) {
  ...
  //TiXmlElement* xmlOrientation = inputElem->FirstChildElement(SML_ORIENTATION);

}

ESP_SML_Location::ESP_SML_Location(TiXmlElement* inputElem,
                                   ESP_SML_Map* map):ESP_SML_Base(inputElem,map) {
  createXml(SML_XML_LOCATION);
  smlVector = new ESP_SML_Vector(inputElem,map);
}

ESP_SML_Location::~ESP_SML_Location() {
  delete smlVector;
}
...
ESP_SML_CommandResponse::ESP_SML_CommandResponse(TiXmlElement* inputElem,
    ESP_SML_Map* mapT):ESP_SML_List(inputElem,mapT,SML_XML_CMD_RESPONSE) {
...
ESP_SML_CmdParam::ESP_SML_CmdParam(std::string type, TiXmlElement* inputElem,
                                   ESP_SML_Map* mapT):ESP_SML_List(inputElem,mapT,type.c_str()) {
...
/**
This class needs a reference to XmlConverted so that it can add the attribute when setValue is invoked
*/
