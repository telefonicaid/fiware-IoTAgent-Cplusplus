...
#define XMLNS_PAID_COMMAND "urn:ogc:def:dictionary:PAID:1.0:paid:PaidCommand.xsd"
...
/**
* This map will contain pointers to objects of ESP_SML_Base and its derived classes so that, when
the transformation is done (within "execute" method at Plugin_postprocessor_SensorML) it will
...
class ESP_SML_Map {
  public:
    ...
    //This method is used to replace "fixed" or default SensorML value with referenced value
    //from parser
...
class ESP_SML_CmdName : public ESP_SML_Base {
  public:
    ESP_SML_CmdName(TiXmlElement* commandResponseXml, std::string refName,
                    ESP_SML_Map* mapT);
    ~ESP_SML_CmdName() {}; //TinyXML objects are destroyed when Tiny XML Document is freed up.
    void setValue(std::string value);
...
class ESP_SML_CmdParam : public ESP_SML_List {
  public:
    ESP_SML_CmdParam(std::string type, TiXmlElement* inputElem,ESP_SML_Map* mapT);
    ~ESP_SML_CmdParam() {};
