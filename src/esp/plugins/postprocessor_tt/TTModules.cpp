...
TTModules::TTModules(std::string n) {
  name.assign(
    n); //This is literally what goes in "name" of the produced JSON (in most of the cases).
  bValid = false;
...
bool TTModules::isValid(){
  return bValid;
}


std::string TTModules::toJSON() {
  return json;
}

std::string TTModules::toPlainJSON() {
  return json;
}



void TTModules::setSleepCond(std::string& s) {
  sleepCondition.assign(s);
}

void TTModules::setSleepTime(std::string& time) {
  sleepTime.assign(time);
}
...
void TTGPSModules::populateInternalMappings(TiXmlElement* element) {
  ...
  CC_Logger::getSingleton()->logDebug("TTGPSModules: populateInternalMappings ...");
  ...
  if (refCoordX != "" && refCoordY != "" && separator != "" && metaName!=""
      && metaType !="" && metaValue !="") {
    CC_Logger::getSingleton()->logDebug("TTGPSModules: populateInternalMappings ... ALL DONE");
  }
  else {
    CC_Logger::getSingleton()->logError("TTGPSModules: populateInternalMappings ... ERROR ,some internal mappings are empty");
  }
...
void TTGPSModules::transform(CC_AttributesType* attributes) {
  ...
  if (itCoordX != attributes->end() && itCoordY != attributes->end() && "" != itCoordX->second.getValueAsString() && "" != itCoordY->second.getValueAsString()) {
    ...
    json.append("\"}");
    //GPS has its own metadatas, so in this case, I need to append sleepCond and SleepTime.
    ...
    CC_Logger::getSingleton()->logError("TTGPSModules: ERROR ,[%s] or [%s] not found in incoming CC_AttributesType",
...
void TTP1_BModules::populateInternalMappings(TiXmlElement* element) {
  ...
  CC_Logger::getSingleton()->logDebug("TTP1_BModules: populateInternalMappings ...");
  ...
  while (xmlMapping) {
    ...
    CC_Logger::getSingleton()->logDebug("Adding mapping: [%s]",nameParam.c_str());
...
void TTP1_BModules::transform(CC_AttributesType* inputMap) {
  ...
  for (std::map<std::string,tt::Pair*>::iterator itMap = mappings.begin() ;
    ...
    if (itInput != inputMap->end() && "" != itInput->second.getValueAsString()) {
      if (count > 0) {
        jSonValue.append("},{");
      }
      ESP_Attribute valueESP = itInput->second;

      CC_Logger::getSingleton()->logDebug("TTModules: checking map: one hit found! [%s]",
                                          itMap->first.c_str());
      ...
      count++;
            ...
            //Do something about missing parameter, like deeming the TTAttribute not valid.
            bValid = false;
            CC_Logger::getSingleton()->logError("TTModules: checking map: NOT found! [%s]",
                                          itMap->first.c_str());
      }
...
void TTGM_Modules::transform(CC_AttributesType* attributes) {
  ...
  if (itName == attributes->end()) {
    CC_Logger::getSingleton()->logError("WARNING: missing [param_1] from incoming message.");
    bValid = false;
  }
  else {
...
std::string TTGC_Modules::toPlainJSON() {
  return plainJson;
}
